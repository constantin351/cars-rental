...
const Modal = ({
  ...
  return createPortal(
    <div onClick={onOverlayClick} className={css.modal}>
      <div className={css.ModalContainer}>
        <div className={css.ContentWrapper}>
          ...
          <div>
            <h2 className={css.modelMakeYearText}>
              {make} <span className={css.blueModelName}>{model},</span> {year}
            </h2>
          </div>
          ...
          <ul className={css.accessList}>
            {accessories.map((accessory, index) => (
              <li key={index} className={css.accessListItem}>
                {accessory}
              </li>
            ))}
          </ul>
          ...
          <ul className={css.functionsList}>
            {functionalities.map((functionality, index) => (
              <li key={index} className={css.functionListItem}>
                {functionality}
              </li>
            ))}
          </ul>
          ...
          <ul className={css.conditionsList}>
            <li className={css.conditionsListItem}>
              Minimum age: <span className={css.blueAge}>{minAge}</span>
            </li>
            <li className={css.conditionsListItem}>
